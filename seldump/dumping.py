...
class Dumper:
    ...
    def _end_table(self, table):
        ...
        if self._copy_size is not None:
            if self._copy_size >= 1024:
                pretty = f" (%s)" % pretty_size(self._copy_size)
            else:
                pretty = ''

            self.write(
                f"-- {self._copy_size} bytes written"
                f" for table {table.escaped}{pretty}\n\n"
            )
    ...
    def begin_dump(self):
        self.write(
            f"-- PostgreSQL data dump generated by pg_seldump {VERSION}\n"
        )
        self.write(f"-- {PROJECT_URL}\n\n")

        self._start_time = now = datetime.utcnow()
        self.write(f"-- Data dump started at {now}Z\n\n")
    ...
    def end_dump(self):
        ...
        self.write(f"-- Data dump finished at {now}Z ({elapsed})\n\n")
